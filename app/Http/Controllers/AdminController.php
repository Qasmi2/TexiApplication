...
class AdminController extends Controller
{
    ...
    {
       //return view('admin.cancel.user');
        try {
            $cancelled = UserRequests::where('status','CANCELLED')->where('cancelled_by','USER')
                ->has('user')
                ->has('provider')
                ->get();
          //  $Reviews = UserRequestRating::where('user_id', '!=', 0)->has('user', 'provider')->get();
        //   var_dump(json_encode($cancelled));
        //   exit();
            return view('admin.cancel.user',compact('cancelled'));
        } catch(Exception $e) {
            return redirect()->route('admin.setting')->with('flash_error','Something Went Wrong!');
        }
    }
    /**
     * provider cancel Requests.
     *
     * @return \Illuminate\Http\Response
     */
    public function provider_cancel()
    {
        //return view('admin.cancel.provider');
        try {
            $cancelled = UserRequests::where('status','CANCELLED')->where('cancelled_by','PROVIDER')
            ->has('user')
            ->has('provider')
            ->get();
          //  $Reviews = UserRequestRating::where('user_id', '!=', 0)->has('user', 'provider')->get();
        //   var_dump(json_encode($cancelled));
        //   exit();
            return view('admin.cancel.provider',compact('cancelled'));
        } catch(Exception $e) {
            return redirect()->route('admin.setting')->with('flash_error','Something Went Wrong!');
        }
    }
